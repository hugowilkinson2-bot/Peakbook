...
export function AdventureDetailScreen({ id }: { id: string }) {
  ...
  const heroSrc = cover?.url ?? adventure.peak?.fotoPrincipalUrl ?? "/peakbook-hero.png";
  ...
  return <AppShell>
    ...
    <article className="overflow-hidden rounded-[2rem] bg-white shadow-card">
      <section className="relative min-h-[25rem] overflow-hidden md:min-h-[31rem]"><ResponsivePhoto src={heroSrc} fallbackSrc="/peakbook-hero.png" alt={`Paisaje de ${adventure.peak?.nombre ?? adventure.titulo}`} priority sizes="(max-width: 1152px) 100vw, 1056px" className="object-cover"/><div className="absolute inset-0 bg-gradient-to-t from-black/90 via-black/15 to-black/10"/><div className="absolute inset-x-6 bottom-6 text-white md:inset-x-8 md:bottom-8"><span className="rounded-full border border-white/20 bg-black/20 px-3 py-1.5 text-[10px] font-bold uppercase tracking-[.14em] backdrop-blur-xl">{formatDifficulty(adventure.dificultad)}</span><h1 className="mt-4 max-w-3xl text-4xl font-semibold leading-none tracking-[-.06em] md:text-6xl">{adventure.peak?.nombre ?? adventure.titulo}</h1><p className="mt-3 flex items-center gap-2 text-sm text-white/65"><CalendarDays size={15}/>{formatDate(adventure.fecha)}{adventure.peak && <> · {adventure.peak.altitud.toLocaleString("es-ES")} m</>}</p></div></section>

      <div className="space-y-7 p-5 sm:p-7 md:p-9">
        {adventure.peak && <section className="flex flex-col gap-3 rounded-[1.5rem] border border-forest/10 bg-forest/[.04] p-5 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-[10px] font-bold uppercase tracking-[.15em] text-moss">Cima identificada</p><h2 className="mt-1 text-lg font-semibold">{adventure.peak.nombre} · {adventure.peak.altitud.toLocaleString("es-ES")} m</h2><p className="mt-1 text-xs text-ink/45">{adventure.peak.provincia ? `${adventure.peak.provincia}, ` : ""}{adventure.peak.pais}</p></div><p className="text-[11px] text-ink/35">{adventure.peak.latitud.toFixed(5)}, {adventure.peak.longitud.toFixed(5)}</p></section>}
        ...
        <AdventureGallery photos={adventure.photos} title={adventure.peak?.nombre ?? adventure.titulo}/>
