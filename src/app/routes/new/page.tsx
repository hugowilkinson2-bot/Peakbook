...
export default function NewRoutePage() {
  return <AppShell><ScreenHeader eyebrow="Añade un nuevo recuerdo" title="Nueva ruta" description="Importa el recorrido y completa los detalles esenciales."/>
    <form className="mx-auto max-w-4xl space-y-5">
      <div className="grid gap-5 md:grid-cols-[.9fr_1.1fr]">
        ...
        <div className="space-y-4"><Field icon={Mountain} label="Nombre de la ruta" placeholder="Ej. Ascenso al Posets"/><Field icon={MapPin} label="Ubicación" placeholder="Parque Natural Posets-Maladeta"/><div className="grid grid-cols-2 gap-4"><Field icon={Route} label="Distancia" placeholder="0,0 km"/><Field icon={CalendarDays} label="Fecha" placeholder="01/08/2026"/></div></div>
      </div>
      ...
      <button type="submit" className="tap-scale flex h-14 w-full items-center justify-center gap-2 rounded-[1.15rem] bg-forest font-semibold text-white shadow-float">Guardar ruta <ArrowRight size={18} strokeWidth={1.9}/></button>
    </form>
  </AppShell>;
}
