...
import { formatDistance, formatDuration, formatElevation } from "@/features/adventures/presentation/formatters";
...
export default function Home() {
  const latest = adventureSummaries[0];
    ...
    <div className="stagger-in">
      <section className="premium-card relative min-h-[31rem] overflow-hidden rounded-[2rem] bg-forest shadow-[0_26px_80px_rgba(14,30,22,.2)] md:min-h-[35rem]">
        ...
        <div className="absolute inset-x-5 top-5 flex items-center justify-between md:inset-x-7 md:top-7">
          <span className="rounded-full border border-white/20 bg-black/20 px-3.5 py-2 text-[10px] font-bold uppercase tracking-[.18em] text-white backdrop-blur-xl">Última aventura</span>
          <Link href="/adventures" aria-label="Ver detalle de la última aventura" className="tap-scale grid h-11 w-11 place-items-center rounded-full border border-white/25 bg-white/90 text-ink shadow-float backdrop-blur"><ArrowRight size={18} strokeWidth={1.9}/></Link>
        </div>
        <div className="absolute inset-x-5 bottom-5 text-white md:inset-x-7 md:bottom-7">
          <div className="mb-3 flex items-center gap-1.5 text-xs text-white/68"><MapPin size={14} strokeWidth={1.9}/>{latest.lugar}</div>
          <h1 className="max-w-xl text-[2.55rem] font-semibold leading-[.98] tracking-[-.06em] md:text-6xl">{latest.titulo}</h1>
          <div className="mt-6 grid grid-cols-3 divide-x divide-white/15 rounded-[1.3rem] border border-white/15 bg-white/10 px-2 py-4 backdrop-blur-xl md:max-w-xl">
            <HeroMetric icon={Route} value={formatDistance(latest.distancia)} label="Distancia"/><HeroMetric icon={Mountain} value={formatElevation(latest.desnivelPositivo)} label="Desnivel"/><HeroMetric icon={Clock3} value={formatDuration(latest.tiempo)} label="Tiempo"/>
          </div>
        </div>
      </section>
      ...
      <section className="mb-8"><SectionTitle eyebrow="Tu archivo" title="Aventuras recientes" action={<Link href="/adventures"><IconButton icon={ArrowRight} label="Ver todas las aventuras"/></Link>}/><div className="grid gap-5 md:grid-cols-2"><AdventureCard adventure={adventureSummaries[1]} compact/><AdventureCard adventure={adventureSummaries[2]} compact/></div></section>
